...
The repository has no TypeScript toolchain or `npm run typecheck`, so the implementation follows the existing dependency-free Node ESM convention with JSDoc contracts rather than adding infrastructure for one pre-live module. A4.2 Exa request count is **0**, cumulative Exa request count remains **2**, and NVIDIA live selector behavior is untested. See `docs/PHASE_A_SELECTOR.md`. Exit status: **`A4.2 PRE-LIVE READY`** pending human review; do not execute it live or begin the representative benchmark without separate approval.
