...
8. A first live-gate CLI attempt for the authorized B5 integrated NVIDIA case stopped at credential preflight (0 network-backed executions). A second, network-backed attempt reached the real pipeline: B1–B3 integrated correctly, but B4B rejected the description and the harness failed to capture why (a harness gap, since fixed). A third, network-backed NVIDIA attempt then **passed** end-to-end. B5 is now `B5 INTEGRATED LIVE GATE — PASSED FOR ONE KNOWN-GOOD COMPANY (NVIDIA)`, not yet broadly validated. Do not run another NVIDIA attempt, another company, endpoint/UI, deployment, or Phase C without a new full-context authorization.

No B3/source verification, synthesis, endpoint, UI, deployment, or other later-phase work occurred during the historical B2 experiments; B3 implementation began only under its subsequent separate authorization. B4A did not begin B4B, B5, or Phase C. B4B's live-gate approval did not begin B5 or Phase C. B5 Stage A did not begin an integrated live gate, endpoint/UI, deployment, or Phase C. Neither the B5 preflight block, the first network-backed attempt, nor the passed second network-backed attempt began broader Phase B validation, endpoint/UI, deployment, or Phase C.
