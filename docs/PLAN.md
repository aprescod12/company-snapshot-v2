...
**A4.3 remains `A4.3 SIGNAL BENCHMARK FAIL`, A4.3R0 remains `NO POLICY CHANGE YET`, and A4.3R1 remains `RETRIEVAL REPAIR FAIL`. A4.5 validated the frozen conditional official-domain fallback on Stripe, so Phase A is `PHASE A ARCHITECTURE APPROVED FOR IMPLEMENTATION`. B1 provides the provider-independent targeting/identity-safety boundary; its live gate safely clarified Mercury ambiguity and positively resolved Stripe with the same broad request shape. B2 remains unstarted. The small direction remains one broad Exa discovery pass, bounded source verification/backfill, and at most one fallback. Cumulative Exa requests are 10.**
