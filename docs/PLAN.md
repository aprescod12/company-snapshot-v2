...
**2026-09-10 — Phase E2 production preflight: E2 BLOCKED — PROJECT-OWNER DECISION REQUIRED.** After the read-only E1 audit found `NO RUNTIME REPAIR JUSTIFIED`, E2 verified local/remote `main` alignment at `d90f497651f62c9de3eaa2e701f459bb1b26efe3` (Vercel deployment metadata not independently observable — no CLI, MCP `403`), confirmed static production health, and made the one authorized `Adobe` (name input) production submission. It reached `insufficient_evidence` with 2 signals that, on manual review, cover the **same** underlying Adobe Q3 FY2026 earnings event — a real distinctness finding, directly analogous to the pre-D3 Linear case. Running the actual D3 merge code locally against the real captured article HTML showed the rule's own conditions (≥2 shared quantity anchors, headline corroboration) were correctly evaluated and correctly did not fire (only 1 shared anchor was extracted, below threshold) — not a logic bug, but a new, previously undocumented failure mode: one publisher's page embeds large non-editorial JSON ahead of article prose, starving the lede-scan window, and separately uses abbreviated `"$6.76B"` notation the anchor regex does not match. Per E2's explicit no-repair protocol, this was documented only; no runtime file changed. Full detail: `docs/PHASE_E2_PRODUCTION_PREFLIGHT.md` and `docs/TESTING.md`'s "Phase E2 — Production Preflight" section.

**2026-09-10 — Phase E2R1 bounded verified-evidence duplicate repair: implemented and zero-network tested, local commit pending project-owner review.** Re-tracing E2's finding through executable evidence (not the original narrative) found the true root cause was different and more precise than first hypothesized: not abbreviated notation, but a nested `<article>` tag silently truncating one publisher's extracted body before its real prose (a generic HTML-matching bug, not Adobe-specific), combined with a numeral-vs-spelled-out-word mismatch ("1 billion" vs. "one billion") independently blocking headline corroboration. Both were proven jointly necessary against the real captured HTML before any code was written. Two narrow, generic corrections were implemented in `src/verification/verifyCompany.mjs`: a tag-depth-counting `balancedTagBodyHtml()` replacing the naive non-greedy `<article>`/`<main>` regex, and a bounded spelled-out-cardinal (`one`–`twenty`) normalization in `collectQuantityAnchors()`. Independent adversarial and diff review before commit found and led to correcting two real safety gaps in the first version — the tag scanner could overrun past a true closing tag if a literal unbalanced `<article`-shaped substring appeared inside an embedded `<script>` payload, and the word-number pattern misread compound words like "twenty-one billion" as "one billion" — both fixed (script/style/comment-aware scanning; a `(?<![\w-])` guard) and covered by new red-before-green regressions. `MINIMUM_SHARED_QUANTITY_ANCHORS`, headline corroboration, `selectSignals.mjs`, B1, B2, orchestration, the endpoint, and the frontend were untouched. Full suite (at that point): 368/368 (361 baseline + 7 new). **No live provider/company request occurred in E2R1**; a production replay requires separate authorization and has not been requested. Full detail: `docs/TESTING.md`'s "Phase E2R1" section.

**2026-09-10 — Project-owner actual-diff review of commit `4f623ca` found two further real gaps, both corrected in a follow-up local commit.** (1) `balancedTagBodyHtml()`'s script/style/comment exclusion only applied once its depth-counting scan began, but a separate, unaware initial-opener search ran first and could itself select a fake `<tagName...>`-shaped string inside an earlier `<script>` block as the "opening" tag — reproduced directly against `4f623ca` with a minimized fixture. (2) word-form quantity anchors were unconditionally tagged `:count`, missing the realistic prose form "one billion dollars" (vs. the assumed, unrealistic "$one billion"), so a currency amount could wrongly share an anchor with an unrelated count — also reproduced against `4f623ca`. Both corrected: `balancedTagBodyHtml()` rewritten as a single unified pass with no separate initial search, so exclusion applies uniformly from the start; `NUMBER_WORD_PATTERN` now captures an optional trailing `dollar`/`dollars` suffix and tags `:currency` when present, normalizing to the same anchor key as the existing numeral currency form. Four new red-before-green regressions confirmed failing against `4f623ca` for exactly the stated reasons, passing after correction. A zero-network causal diagnostic against the real captured Adobe HTML (no new fetch) confirmed the old code left the pair non-duplicate (matching the original production observation) and the corrected code recognizes it as duplicate coverage. Full suite: 372/372. No live provider/company request occurred. Full detail: `docs/TESTING.md`'s "Project-owner review corrections" subsection.

**2026-09-10 — One authorized post-correction production replay (`Adobe`, name input, commit `d9095235cfccf8d3a91397c9453fabf726073b2a`) reproduced the same outcome as the original E2 observation**: the same two publisher pages were again displayed as two separate signals, despite the zero-network causal diagnostic confirming the corrected code recognizes this exact pair as duplicate coverage against the captured HTML. No further live investigation was authorized. **Final disposition: the residual production duplicate is accepted as a known, disclosed live-web/publisher-structure limitation; no E2R2 investigation was pursued or authorized; runtime/retrieval work is frozen for submission** absent a new concrete blocker. The E2R1 repair remains correct and verified against the evidence it was built for. Final cumulative provider accounting: Search 57–68 / Contents 19 / retries 0. Full detail: `docs/TESTING.md`'s "Live replay status" section.
