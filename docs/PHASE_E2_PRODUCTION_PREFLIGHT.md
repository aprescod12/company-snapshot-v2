...
Project-owner actual-diff review of that E2R1 commit then found two further real gaps (an initial-opener exclusion gap and a spelled-out currency/count normalization gap), both reproduced and corrected in a follow-up commit — full suite 372/372, zero-network causal diagnostic confirmed against the real captured Adobe HTML. See `docs/TESTING.md`'s "Project-owner review corrections" subsection.

One authorized post-correction production replay (`Adobe`, name input) then reproduced the same outcome as the original finding above — the same two publisher pages were again displayed as two separate signals in production, despite the corrected code recognizing this exact pair as duplicate coverage against the captured HTML. **Final disposition:** accepted as a known, disclosed live-web/publisher-structure limitation; no further investigation (E2R2) was pursued or authorized; runtime/retrieval work is frozen for submission. See `docs/TESTING.md`'s "Live replay status" section for the full record.
