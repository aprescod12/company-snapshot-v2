...
**Phase A is `PHASE A ARCHITECTURE APPROVED FOR IMPLEMENTATION`: one broad Exa discovery pass, bounded verification, and at most one official-domain fallback. B1's deterministic company-targeting boundary is complete; its one-request Mercury live identity gate stopped safely on a raw-result format failure, and B2 remains unstarted. A4.3 remains `A4.3 SIGNAL BENCHMARK FAIL`; A4.3R0 remains `NO POLICY CHANGE YET`; and A4.3R1 remains `RETRIEVAL REPAIR FAIL`.**
...
B1 adds a dependency-free local boundary for user company targeting and later identity confirmation. It treats domain/URL input as a normalized hostname anchor, keeps name input provisional without guessing a domain, and keeps ordinary whitespace-containing punctuated company names on the name path. A name target needs compatible corroborated identity evidence plus explicit `ambiguous: false`; it does not require its official domain to resemble the company name. The separately authorized gate made one Mercury request with the unchanged A4.1 body plus only the identity schema, then safely stopped because returned `results[9]` lacked a non-empty title. No identity output, grounding, source, cost, or latency conclusion was retained; Stripe was not requested. B1 therefore does not establish live Mercury behavior, makes cumulative Exa requests 8, and does not include B2 discovery.
