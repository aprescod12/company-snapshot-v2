...
B1 adds a dependency-free local boundary for user company targeting and later identity confirmation. It treats domain/URL input as a normalized hostname anchor, keeps name input provisional without guessing a domain, and keeps ordinary whitespace-containing punctuated company names on the name path. A name target needs compatible corroborated identity evidence plus explicit `ambiguous: false`; it does not require its official domain to resemble the company name. This makes B1 a safe evidence boundary rather than a semantic Mercury resolver: the Mercury / `shipmercury.com` live identity gate remains pending. B1 made no provider request, leaves cumulative Exa experimental requests at 7, and does not include B2 discovery.
