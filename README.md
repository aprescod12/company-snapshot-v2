...
B4B Stage A adds `src/description/exaCompanyDescription.mjs`, an isolated candidate for the description B4A intentionally does not generate. It is deliberately kept separate from B2's existing broad-discovery `outputSchema` so a description feature cannot risk the approved Search/identity/signal-retrieval behavior. It makes one Exa **Contents** request (never Search) against the exact HTTPS root of an already-confirmed official domain, requesting `summary` only, and validates the result structurally: 2–3 sentences, bounded length, no embedded URL, and a returned URL on the confirmed domain or a subdomain. It does not import B1, B2, B3, or B4A. Stage A made 0 live Exa Search or Contents requests; a bounded future live-gate harness exists but was not run. B4B is an unapproved isolated production candidate pending a separately authorized live validation gate; endpoint/UI wiring (B5) and the frontend (Phase C) have not begun.
