...
**Phase A is `PHASE A ARCHITECTURE APPROVED FOR IMPLEMENTATION`: one broad Exa discovery pass, bounded verification, and at most one official-domain fallback. B1's deterministic company-targeting boundary is complete and its live gate passed. B2 is `B2 PRODUCTION BROAD DISCOVERY — APPROVED`. B3 has owner-run live results but remains not production-approved pending diagnostic root-cause review and a project-owner decision. Cumulative Exa experimental requests are 18. A4.3 remains `A4.3 SIGNAL BENCHMARK FAIL`; A4.3R0 remains `NO POLICY CHANGE YET`; and A4.3R1 remains `RETRIEVAL REPAIR FAIL`.**
...
B3 consumes B2's complete prioritized queue in order and verifies exact publisher URLs using bounded native GET retrieval: HTML/XHTML only, manual redirects capped at five, one five-second deadline across redirects, headers, and body, a 2 MiB body cap, and no cookies, browser automation, retry, crawl, or literal-IP target. It derives a publisher headline, compact supporting passage, and source publication date (JSON-LD `datePublished`, article metadata, then `<time datetime>`); provider dates cannot prove eligibility. It rejects blocked, unsupported, undated/stale, trivial, and duplicate evidence, stops at three accepted records, and conditionally makes one A4.5-role raw fallback Search—resolved company name plus official-domain root/wildcard constraint, not B2 identity output—only after the broad queue is exhausted below three. Its initial key-missing preflight remains historical, but the owner subsequently ran NVIDIA (broad plus fallback; two accepted records and `insufficient_evidence`) and Stripe (B2 clarification; B3 not reached), plus one Stripe B2 diagnostic request. Those four actual Exa requests bring the cumulative count to 18. A smoke-only diagnostic observer now records safe B2 identity diagnostics and the actual B3 candidate decisions without changing production behavior. B3 remains not production-approved pending diagnostic root-cause review and project-owner decision.
