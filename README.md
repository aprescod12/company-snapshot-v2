...
**Phase A is `PHASE A ARCHITECTURE APPROVED FOR IMPLEMENTATION`: one broad Exa discovery pass, bounded verification, and at most one official-domain fallback. B1's deterministic company-targeting boundary is complete; its second, parser-corrected Mercury gate safely clarified two entities, and B2 remains unstarted. A4.3 remains `A4.3 SIGNAL BENCHMARK FAIL`; A4.3R0 remains `NO POLICY CHANGE YET`; and A4.3R1 remains `RETRIEVAL REPAIR FAIL`.**
...
B1 adds a dependency-free local boundary for user company targeting and later identity confirmation. It treats domain/URL input as a normalized hostname anchor, keeps name input provisional without guessing a domain, and keeps ordinary whitespace-containing punctuated company names on the name path. A name target needs compatible corroborated identity evidence plus explicit `ambiguous: false`; it does not require its official domain to resemble the company name. The first live gate revealed that Exa titles can be absent, so only title metadata became optional while valid HTTP(S) URLs remained required. The one authorized Mercury rerun returned the fintech and Mercury Systems entities with `ambiguous: true`; B1 safely clarified rather than accepting either. Mercury live requests are 2, Stripe is 0, cumulative Exa requests are 9, and B2 remains unstarted.
